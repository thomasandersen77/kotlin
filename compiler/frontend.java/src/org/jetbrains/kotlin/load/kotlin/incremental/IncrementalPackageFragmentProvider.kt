...
class IncrementalPackageFragmentProvider(
    ...
    inner class IncrementalPackageFragment(fqName: FqName) : PackageFragmentDescriptorImpl(moduleDescriptor, fqName) {
        ...
        fun getPackageFragmentForMultifileClass(multifileClassFqName: FqName): IncrementalMultifileClassPackageFragment? {
            val facadeInternalName = JvmClassName.byFqNameWithoutInnerClasses(multifileClassFqName)
            val partsNames = incrementalCache.getStableMultifileFacadeParts(facadeInternalName.internalName) ?: return null
            return IncrementalMultifileClassPackageFragment(facadeInternalName, partsNames, multifileClassFqName.parent())
        }
