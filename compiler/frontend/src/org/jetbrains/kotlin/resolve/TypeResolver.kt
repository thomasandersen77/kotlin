...
class TypeResolver(
    ...
    private fun getScopeForTypeParameter(c: TypeResolutionContext, typeParameterDescriptor: TypeParameterDescriptor): MemberScope {
        return when {
            c.checkBounds -> IntersectionTypeUtils.getUpperBoundsAsType(typeParameterDescriptor).memberScope
            else -> LazyScopeAdapter(LockBasedStorageManager.NO_LOCKS.createLazyValue {
                IntersectionTypeUtils.getUpperBoundsAsType(typeParameterDescriptor).memberScope
            })
        }
    }
